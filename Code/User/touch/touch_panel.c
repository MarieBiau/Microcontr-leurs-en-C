...
void touch_debug(void)
{
    ...
    while( TP_DOWN() )
    {
        ...
        //display
        {
            ...
            tprintf("touch down! X:%04d Y:%04d\r\n",touch_x,touch_y);
        }
    }
...
void touch_down(uint16_t *touch_x,uint16_t *touch_y,int *appui)
{
		uint16_t tmpx[10];
    uint16_t tmpy[10];
    unsigned int i;
    ...
    while( TP_DOWN() )
    {
        ...
        //display
        {
            ...
            sprintf(x_str,"X: %04d",*touch_x);
            sprintf(y_str,"Y: %04d",*touch_y);
            LCD_write_english_string(0,0,x_str,Cyan,Cyan);
            LCD_write_english_string(0,0,y_str,Cyan,Cyan);
            tprintf("touch down! X:%04d Y:%04d\r\n",touch_x,touch_y);
        }
				*appui=1;
    }
